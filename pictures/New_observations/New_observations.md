...
data1['sex'].replace(to_replace = ['Female', 'female', 'I identify as female.', 'female ',
...
data1['sex'].replace(to_replace = ['Bigender', 'non-binary', 'Other/Transfeminine',
...
#In this data, we don't have actually as much missing values as original data.

plt.figure(figsize = (16,4))
sns.heatmap(data = data1.isna());
```
...
```python
# The survey has 50 rows, so first we will drop all columns where more than half of the observations have missing values(if have)
cols = (data1.isna().sum() >= data1.shape[0]/2).tolist()
to_drop = data1.columns[cols]
data1.drop(labels = to_drop, axis = 1, inplace = True)
...
```python
# ----------- OTHER CHANGES -----------
# There is just 1 country, Poland.
# That's why we don't use here Countries to plot as original data, because it won't show us proper result.
# 

# Keep only countries with no. responses > 30.
#imp_data = imp_data[imp_data['country_work'].isin(['Poland'])]
#imp_data = imp_data[imp_data['country_live'].isin(['Poland'])]

#prep_data = prep_data[prep_data['country_work'].isin(['Poland'])]
#prep_data = prep_data[prep_data['country_live'].isin(['Poland'])]
...
# As other countries, here also most of participants are Male.
