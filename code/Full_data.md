...
full_data['sex'].replace(to_replace = ['Female', 'female', 'I identify as female.', 'female ',
...
full_data['sex'].replace(to_replace = ['Bigender', 'non-binary', 'Other/Transfeminine',
...
full_data['country_live'].replace(to_replace = ['United Kingdom'], value = 'UK', inplace = True)
full_data['country_work'].replace(to_replace = ['United States of America'], value = 'USA', inplace = True)
full_data['country_work'].replace(to_replace = ['United Kingdom'], value = 'UK', inplace = True)
...
mean_age = full_data[(full_data['age'] >= 18) | (full_data['age'] <= 75)]['age'].mean()
full_data['age'].replace(to_replace = full_data[(full_data['age'] < 18) | (full_data['age'] > 75)]['age'].tolist(),
...
sns.heatmap(data = full_data.isna());
```
...
```python
# Now this dataset has 1483 rows, so again first we will drop all columns where more than half of the observations have missing values.

cols = (full_data.isna().sum() >= full_data.shape[0]/2).tolist()
to_drop = full_data.columns[cols]
full_data.drop(labels = to_drop, axis = 1, inplace = True)
...
```python
#In original data, we had only 4 participant from Poland. Now with added new observations, it already started to show up on results
#as for Poland, we already have more than 30 results.
```
...
```python
# It seems that majority number of males among participants is also the same for Poland as well. But because of number of new observations
# compared to original data is too small, it does't affect to result much, but on average 0.4% more for each.
```
